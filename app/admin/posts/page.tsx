  ...
  const loadPosts = async (reset = false) => {
    try {
      ...
      const { posts: newPosts, lastVisible } = await getAdminPosts({
        ...
        status: status === "published" ? "published" : status === "draft" ? "draft" : undefined,
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Edit, delete, and manage news content visibility.</p>
        </div>
        <Link 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Article</th>
            ...
            <tbody className="divide-y divide-gray-100 text-black">
              {loading ? (
                    ...
                    <td className="p-4 min-w-[300px]">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-bold text-sm text-gray-900 group">
                          <Link href={`/news/${post.slug}`} target="_blank" className="hover:text-red-600 flex items-center gap-1">
                            {post.title}
                    ...
                    <td className="p-4">
                      <button 
                        ...
                        className={`inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full transition ${
                          post.isPublished ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-amber-100 text-amber-700 hover:bg-amber-200"
                        }`}
                      >
                        {post.isPublished ? <CheckCircle className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
                ...
                <tr>
                  <td colSpan={5} className="p-12 text-center text-gray-500 italic">No posts found matching your filters.</td>
                </tr>
              )}
            </tbody>
          </table>
